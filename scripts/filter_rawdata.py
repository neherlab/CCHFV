...
def group_metadata(df):
    ...
    df_filtered = df_filtered.drop(
        df_filtered[
            (df_filtered["Segment"] == "S") & (df_filtered["Length"] < min_length_s)
        ].index
    )
    df_filtered = df_filtered.drop(
        df_filtered[
            (df_filtered["Segment"] == "M") & (df_filtered["Length"] < min_length_m)
        ].index
    )
    df_filtered = df_filtered.drop(
        df_filtered[
            (df_filtered["Segment"] == "L") & (df_filtered["Length"] < min_length_l)
        ].index
    )
    ...
    for g in groups:
        ...
        df_grouped["group_id"] = df_grouped.apply(
            lambda row: row["group_id"] + group_id
...
def create_segment_fasta(raw_sequences):
    ...
    with open(sequences_M, "w") as seq_M:
        records = SeqIO.parse(all_sequences, "fasta")
        for record in records:
    ...
    with open(sequences_L, "w") as seq_L:
        records = SeqIO.parse(all_sequences, "fasta")
        for record in records:
